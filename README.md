...
這個專案由**兩支對外的 exe** + 幾個內部模組組成。用一句話定位每個角色：

| 元件 | 角色 | 對外可見 |
|---|---|---|
| `ping.py` | **resetter 的核心引擎**：真正被呼叫去把 Claude 5 小時視窗的重置時鐘催起來的那件事（先花 0 元查用量，必要時才打一次最便宜的探針）。 | 否，內部（由 `--tick` 呼叫） |
| `scheduler.py` | 專門把「到點跑一次」註冊進 Windows 工作排程器的內部工具。 | 否，內部（使用者不會直接碰） |
| `installer.py` → **ClaudeTimerResetter.exe** | **對外可見 #1**：安裝 resetter。把自己複製到安裝目錄、註冊排程；到點由排程把這支 exe 開起來、以 `--tick` 呼叫 `ping`。 | 是 |
| `resumer.py` → **ClaudeResumer.exe** | **對外可見 #2**：獨立功能——排定「重置時間一到，自動把某個被卡住的對話接著送出去」。 | 是 |

其餘 `rules.py`（排程規則引擎）、`usage.py`（零成本查用量）、`tray.py`（工具列常駐驅動）、
`autostart.py`（開機捷徑）都是內部輔助模組，沒有自己的 exe。

一個容易誤會的點：**「安裝器」跟「被排程叫起來跑 ping 的 resetter」是同一支 exe**——
`ClaudeTimerResetter.exe` 靠參數分流（無參數＝開安裝 GUI、`--tick`＝跑一次 `ping`、
`--tray`＝工具列常駐）。所以安裝器安裝的其實就是它自己，之後由排程到點呼叫它跑 `ping`。
...
| `python installer.py --tick` | 靜默跑一次「有沒有規則到期」，到期才刷新 | Windows 工作排程器，到你設定的時間點（例如每天 05/10/15/20）觸發 |
...
- **工作排程器（建議）**：註冊進 Windows Task Scheduler，**到你設定的時間點才把 exe
  開起來跑一次**（有 `daily_times` 規則時就排那幾個時刻的觸發器，不是每分鐘輪詢），跑完
  就關。不用保持任何視窗開著，也不需要系統管理員權限。程式「跑完就關」正是它**打不死**
  的原因——沒有長命行程可以當掉，每次觸發都是全新獨立的一次；電腦睡眠或剛開機時錯過的
  時刻會盡快補跑一次（`StartWhenAvailable`）。
- **工具列常駐**：程式留在系統工具列一直開著（可勾選「開機時自動啟動」），靠內部計時器
  到點觸發。想看目前狀態、立即測試、或結束程式都在右鍵選單。缺點是它是**單一長命行程，
  一旦當掉就會停擺到下次重開機**，可靠度不如排程——所以除非你就是想要一個看得見的圖示，
  一般建議用工作排程器。
...
2. 上方清單會列出本機（`~/.claude/projects/`）所有對話，依最後活動時間排序，可以打字
   ...
   `● 開啟中`**（見下面「怎麼保證不會被佔用」）
3. 不確定是不是那一個？**雙擊該列（或按「查看內容」）先預覽對話內容**，不花錢，避免
   選錯——預覽視窗裡確認無誤可以直接按「就選這個對話」帶回清單
4. 填要接續送出的內容（預設 `continue`，可以自己改，也可以用「附加檔案」把檔案路徑
   用 `@` 語法帶進 prompt）、設定時間、選「自動送出」或「時間到手動確認再送出」
5. 「加入排程佇列」——可以一次排多筆

### 幾個操作上的細節

- **先預覽再選，不會選錯**：對話清單上雙擊任一列、或選起來按「查看內容」，會開一個
  唯讀小視窗把那個對話的往來內容攤開來看（只讀本機 `.jsonl`，**不花 token**）。很長的
  對話會只顯示「第一則＋最後一段」、中間用省略號帶過，開啟時自動捲到最近的訊息（通常
  就是被 session limit 卡住的地方）。確認是對的就按「就選這個對話」，會直接在主畫面選好。
...
**主動搶占（「強制接管並送出」按鈕）**：如果你不想等自己去關 VSCode，佇列那排有一顆
「強制接管並送出」。它會顯示是誰佔用（程式 / pid / 專案）→ 跳確認 → 直接 `taskkill` 那個
持有行程 → 等鎖真的釋放 → 再送出。注意這是**破壞性**的：被強關的那個對話在 VSCode 裡
沒存、或正在跑的東西會一起沒掉（Claude Code 的鎖是「誰的行程還活著誰持有」，沒有禮貌
接管的辦法，只能讓現在的持有者結束）。平常還是建議用上面「關掉就自動補送」的被動流程，
趕時間才用強制接管。
